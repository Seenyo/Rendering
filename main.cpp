...
Camera g_Camera;
const int tri_num = 6;
const int light_source_num = 5;
RectangleObj rects[2];
TriangleObj tris[tri_num];
double tri_area = 0.0;
double pdf[tri_num];
...
void
makeRectangle(const Eigen::Vector3d &centerPos, const Eigen::Vector3d &widthVec, const Eigen::Vector3d &heightVec,
              const Eigen::Vector3d &color, const bool &is_light, const double &kd, RectangleObj &out_Rect) {
...
void makeTriangle(const Eigen::Vector3d &v1, const Eigen::Vector3d &v2, const Eigen::Vector3d &v3,
                  const Eigen::Vector3d &color, const bool &is_light, const double &kd, TriangleObj &out_Tri) {
    out_Tri.v1 = v1;
    out_Tri.v2 = v2;
    out_Tri.v3 = v3;
            ...
            if (ray_hit.mesh_idx >= 0) {
                if (tris[ray_hit.mesh_idx].is_light == true) {
                    ...
                    for (int i = 0; i < sample_num; i++) {
                        int tri_idx;
                        ChoseTriangle(tri_idx);
                        A = tris[tri_idx].area;
                        const float gamma = 1.0 - sqrt(1.0 - drand48());
                        const float beta = drand48() * (1.0 - gamma);
                        const Eigen::Vector3d y =
                                (1.0 - beta - gamma) * tris[tri_idx].v1 + beta * tris[tri_idx].v2 +
                                gamma * tris[tri_idx].v3;
                        ...
                        sum = sum + (intensity * tris[ray_hit.mesh_idx].kd / M_PI *
                                     cosx * cosy / y_x.squaredNorm()) * tris[tri_idx].color;
...
int main(int argc, char *argv[]) {
    g_Camera.setEyePoint(Eigen::Vector3d{0.0, 1.5, 5.5});
    ...
    makeTriangle(Eigen::Vector3d{0.0, 0.0, 1.5},
